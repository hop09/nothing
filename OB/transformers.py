        ...
        new_ast: Module = Module(
            body=[
                Assign(  
                    ...
                    value=List(
                        elts=[x.to_ast_loader() for x in self.found],  
                        ctx=Load()
                    )
                ),
                *ast.body 
...
class IntObfuscator(Transformer, NodeTransformer):
    def visit_Constant(self, node: Constant) -> Any:
        ...
        if type(node.value) == int:
            ic: int = node.value
            is_signed = ic < 0  
            rdx = math.ceil((ic.bit_length() + (1 if is_signed else 0)) / 8) 
            int_bytes = ic.to_bytes(rdx, "little", signed=is_signed)
            off = random.randint(255 + rdx, 999) 
            encoded = "".join([format(off - (x + i), "03d") for (x, i) in zip(int_bytes, range(len(int_bytes)))])
            return Call(  
                func=Attribute(Name('int', Load()), 'from_bytes', Load()),  
                args=[
                    Call(  
                        func=Name('map', Load()),
                        args=[
...
class ConstructDynamicCodeObject(Transformer):
    ...
    def _parse_const(self, el: Any, ctx):
        if isinstance(el, tuple):
            ...
            if el in self.code_obj_dict:  # 
                return Call(
                    func=Name(self.code_obj_dict[el], Load()),
                    args=[],
                    keywords=[]
                )
            else:  # i dont remember what im doing. its hop
                b = marshal.dumps(el)
                return Call(
                    func=Attribute(
                        value=Call(
